...
function import_csv_products(){
    ...
    $step = 10; // import items per step

    $outut_html = '';
    // start table
    if ($current_item == 1) {
    	$outut_html .= '<tr>';
    	$exp_keys = [ 'ID', 'Obrazek', 'Produkt', 'SKU', 'Cena', 'Kategorie'];
    ...
    for ($i=1; $i <= $step ; $i++) {
        ...
        $test = ob_get_clean();
        $current_item++;
        if( $product_data['store_view_code'] !== '' ) continue;
    	if( !$test_mode ){ input_product_exported( $product_data ); }
...
function isu_ma2_product_woo_data( $csv_data, $dir ){
	...
	foreach ($keys as $key => $key_id) {
		if( in_array($key_id, $images_keys) ){
			$wanted_data[$key] = $url_conten.$csv_data[$key_id];
		}else{
			$wanted_data[$key] = $csv_data[$key_id];	
		}
	}
	return $wanted_data;
}
